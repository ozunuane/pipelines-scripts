...
resource "aws_iam_role" "tasks" {
  ...
  name        = var.tasks_iam_role_use_name_prefix ? null : "${var.env}-${var.service_name}task-ex-role"
  name_prefix = var.tasks_iam_role_use_name_prefix ? "${var.env}-${var.service_name}task-ex-role" : null
...
# locals {
#   cluster_aws_ecs_service = aws_ecs_service.this.cluster
...
#   triggers = {
#     task_definition = aws_ecs_task_definition.this.revision
#   }
